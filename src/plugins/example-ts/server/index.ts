...
import { PluginMetadata, PluginLifecycle, CoreAPI } from '../../../core/shared/interfaces';
...
const metadata: PluginMetadata = {
  ...
  version: '1.0.0',
  author: 'AltDragon',
  description: 'An example plugin that demonstrates how to use the core resource',
  dependencies: ['core'],
...
function registerEventHandlers(): void {
  ...
  core.on('core:hotReload:complete', (event) => {
    ...
    alt.setTimeout(() => {
      try {
        ...
        alt.setTimeout(() => {
          ...
          sendNotification(player, `Welcome to the server, ${player.name}! Press F2 or type /ui to toggle the UI`);
          core.log(`Player ${player.name} spawned at ${spawnPos.x}, ${spawnPos.y}, ${spawnPos.z}`);
        }, 1000);
      } catch (error) {
